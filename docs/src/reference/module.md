...
| `nativeCompilerCaches: boolean`   | `true`                           | (Only for [native targets](https://kotlinlang.org/docs/native-target-support.html)) Reuse the compiled native code of external dependencies across builds, which makes linking binaries significantly faster. Only applies to debug (non-optimized) binaries. |
