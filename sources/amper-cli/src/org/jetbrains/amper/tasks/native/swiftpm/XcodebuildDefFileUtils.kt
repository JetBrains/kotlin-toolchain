...
internal fun Path.listFilesOrEmpty() = (if (isDirectory()) this.listDirectoryEntries() else null).orEmpty()
...
internal object XcodebuildDefFileUtils {
    ...
    data class ParsedLdCall(
        val ldArgsForExecutable: List<String>,
        val linkTimeFrameworkSearchPaths: Set<String>,
        val librarySearchPaths: Set<String>,
    ) {
        fun dyldEnvSearchPaths(isSimctlCall: Boolean): Map<String, String> {
            val envPrefix = if (isSimctlCall) "SIMCTL_CHILD_" else ""
            return mapOf(
                envPrefix + "DYLD_FALLBACK_FRAMEWORK_PATH" to linkTimeFrameworkSearchPaths.joinToString(":"),
                envPrefix + "DYLD_FALLBACK_LIBRARY_PATH" to librarySearchPaths.joinToString(":"),
            )
        }
    }
    ...
    fun parseLdCall(architectureSpecificProductLdCall: Path): ParsedLdCall {
        ...
        resplitLdCall.forEachIndexed { index, arg ->
            ...
            if (arg.startsWith("/")) {
                ...
                if (arg.endsWith(".dylib")) {
                    ldArgs.add(arg)
                    librarySearchPaths.add((Path(arg).parent.pathString))
                }

                if (".framework/" in arg) {
                    if (!arg.endsWith("/" + SYNTHETIC_IMPORT_DYLIB)) {
                        ldArgs.add(arg)
                    }
                    linkTimeFrameworkSearchPaths.add(
                        Path(arg).parent.parent.pathString
                    )
                }
            }
        }
        ...
        return ParsedLdCall(
            /**
             * In Gradle we have 2 separate ldArgs lists:
             * 1. For producing dynamic frameworks because we need to correctly link against other dynamic frameworks
             * 2. Another for K/N executables (e.g. for tests) which tries to avoid dynamic frameworks
             *
             * Kotlin Toolchain doesn't have dynamic frameworks, so we have just 1 list here
             */
            ldArgsForExecutable = ldArgs + filelist + argvObjectFiles,
    ...
    fun discoverClangModules(parsedClangCall: ParsedClangCall): Set<String> {
        fun inferModuleName(modulemap: Path): String? = MODULE_NAME_REGEX.find(modulemap.readText())?.let {
            it.groups[1]?.value
        }
        ...
        parsedClangCall.compileTimeFrameworkSearchPaths.map { Path(it) }.filter { it.exists() }.forEach {
        ...
        parsedClangCall.includeSearchPaths.map { Path(it) }.filter { it.exists() }.forEach { searchPath ->
