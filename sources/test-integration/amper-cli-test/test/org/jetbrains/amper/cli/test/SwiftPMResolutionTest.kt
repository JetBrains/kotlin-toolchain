...
import org.jetbrains.amper.tasks.native.swiftpm.SwiftPMDependencies
import org.jetbrains.amper.tasks.native.swiftpm.TransitiveSwiftPMMetadata
...
class SwiftPMResolutionTest : AmperCliTestBase() {
    ...
    fun `disconnected swiftpm dependency`() = runTestWithMdc {
        val resolved = resolveSwiftPMDependencies(
            "swiftpm/disconnected-swiftpm-dependency",
            "consumer"
        )
        assertEquals(
            emptyList(),
    ...
    fun `multiple transitive swiftpm dependencies`() = runTestWithMdc {
        val resolved = resolveSwiftPMDependencies(
            "swiftpm/transitive-swiftpm-dependency",
            "consumer"
        )
        assertEquals(
            emptyList(),
